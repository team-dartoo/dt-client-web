...
const Main = () => {
  ...
  const [myCompanyDisclosures, setMyCompanyDisclosures] = useState([]);
    ...
    const DISCLOSURES = [
      {
        ...
        isNew: true,
        sentiment: "positive",
        company: { companyId: "005930", name: "삼성전자", code: "005930" },
        summary: { status: "loading", lines: [] },
      },
      {
        disclosureId: "2",
        title: "전지 생산 관련 신규 설비 투자 결정",
      ...
      },
      {
        ...
        isNew: false,
        sentiment: "negative",
        company: { companyId: "035720", name: "카카오", code: "035720" },
        summary: {
          status: "success",
          lines: [
            "조직/사업 구조 재편 내용 발표",
      ...
      },
      {
        ...
        isNew: false,
        sentiment: "positive",
        company: { companyId: "005380", name: "현대자동차", code: "005380" },
        summary: { status: "loading", lines: [] },
      },
    ];
    // 1) 오늘의 공시(최근 3개) - 더 빨리 도착한다고 가정
    setLatestLoading(true);
    setTimeout(() => {
      // ✅ 서버라면: fetchLatestDisclosures()
      // ✅ 지금은: DISCLOSURES에서 최근 3개만
      const latest3 = DISCLOSURES.slice(0, 3);

      setLatestDisclosures(latest3);
      setLatestLoading(false);
    }, 500);

    // 2) 내가 찜한 기업 공시(전체) - 좀 늦게 도착한다고 가정
    setMyLoading(true);
    setTimeout(() => {
      // ✅ 서버라면: fetchMyCompanyDisclosures()
      setMyCompanyDisclosures(DISCLOSURES);
      setMyLoading(false);
    }, 2000);
  }, []);
  ...
  return (
    <div className="main page">
          ...
          {latestLoading ? (
            ...
            latestDisclosures.map((item) => (
              <div
                ...
                onClick={() => navigate(`/disclosure/${item.disclosureId}`)}
      ...
      <MyCompanySection
        companies={companies}
        disclosures={myCompanyDisclosures}
        loading={myLoading}
      />
    </div>
  );
